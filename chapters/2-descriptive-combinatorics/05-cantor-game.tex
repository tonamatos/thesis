...
\textcolor{orange}{I want to add a comment connecting this to Section~\ref{sec:ramsey} once it is written}
...
More generally, a well-known result is that Player II has a winning strategy if and only if $S$ is of the first category.
In particular, Player II wins whenever $S$ is countable.

The game I study in this section, \emph{Baker's game}, is similar:
Two players take turns selecting real numbers; Player I tried to ensure that some element in the fixed set $S$ remains in between all of these real numbers.
